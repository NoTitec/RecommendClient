...
public class SignUpController implements Initializable{
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        backButton.setOnAction(event -> handlerSetBackbtnAction(event));
    ...
    private void handlerSetBackbtnAction(ActionEvent event) { //뒤로가기버튼클릭 (씬전환)
    ...
    private void handlerSetSignUpbtnAction(ActionEvent event) {
        ...
        if(IDField.getText().isEmpty() || PWField.getText().isEmpty()){
            ...
            proto = new Protocol(TYPE_REQUEST, CODE_SIGNUP);
            sendPos = 2;
            ...
            try{
                receiveBuf = proto.getPacket(TYPE_RESPONSE_ERROR, CODE_SIGNUP);
                // 기본값 = RESPONSE_ERROR 로 설정
                bis.read(receiveBuf);
                //만약 회원가입을 성공했다면 TYPE = 1(RESPONSE) 일것이고 실패했다면(중복아이디존재) TYPE = 2(RESPONSE_ERROR)로 설정될것이다.
            ...
            if((int)receiveBuf[0] == TYPE_RESPONSE && (int)receiveBuf[1] == CODE_SIGNUP){ //회원가입을 성공했을 경우

                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setContentText("회원가입을 성공했습니다!!!.");
                alert.show();
                IDField.clear();
                PWField.clear();
                IDField.requestFocus();

            }else if((int)receiveBuf[0] == TYPE_RESPONSE_ERROR && (int)receiveBuf[1] == CODE_SIGNUP){ //회원가입 실패했을 경우(중복 아이디 존재)

                Alert alert = new Alert(Alert.AlertType.INFORMATION);
                alert.setContentText("이미 존재하는 아이디입니다. 다시 입력해주세요");
                alert.show();
                IDField.clear();
                PWField.clear();
                IDField.requestFocus();
